...
def fetch_evaluation_schedule_by_ids(service_order_id, employee_id):
    """
    ...
    source_info = {
        ...
        "note": None,
        "error": None,
        "query": f"Fetching evaluation schedule for ServiceOrderID: {service_order_id}, EmployeeID: {employee_id}"
    }
    ...
    try:
        ...
        cursor.execute(query, (service_order_id, employee_id))
        ...
        if row:
            ...
            source_info["note"] = f"Successfully retrieved evaluation schedule for ServiceOrderID: {service_order_id}, EmployeeID: {employee_id}"
        else:
            source_info["note"] = f"No evaluation schedule found for ServiceOrderID: {service_order_id}, EmployeeID: {employee_id}"
        
        # Close connection
        cursor.close()
        conn.close()
        
        print(f"✅ Database query successful - Retrieved evaluation schedule for ServiceOrderID: {service_order_id}, EmployeeID: {employee_id}")
        ...
        source_info.update({
            "source": "mock_data_fallback",
            "error": error_message,
            "note": f"Database connection failed, returning mock evaluation schedule data for ServiceOrderID: {service_order_id}, EmployeeID: {employee_id}"
        })
