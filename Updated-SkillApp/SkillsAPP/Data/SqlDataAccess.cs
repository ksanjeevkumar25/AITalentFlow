    ...
    public class SqlDataAccess
    {
        ...
        public void InsertSkill(Skill skill)
        {
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                int nextSkillId = 1;
                using (var cmdMax = new SqlCommand("SELECT ISNULL(MAX(SkillID), 0) + 1 FROM Skill", conn))
                {
                    nextSkillId = (int)cmdMax.ExecuteScalar();
                }

                using (var cmd = new SqlCommand(@"
            INSERT INTO Skill (SkillID, SkillName, SkillDescription)
            VALUES (@SkillID, @SkillName, @SkillDescription)", conn))
                {
                    cmd.Parameters.AddWithValue("@SkillID", nextSkillId);
                    cmd.Parameters.AddWithValue("@SkillName", skill.SkillName);
                    cmd.Parameters.AddWithValue("@SkillDescription", skill.SkillDescription ?? skill.SkillName);
                    cmd.ExecuteNonQuery();
                }
            }
        }
